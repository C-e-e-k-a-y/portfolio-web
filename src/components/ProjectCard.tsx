      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {projects.map((project, index) => {
          ...
          return (
            ...
            >
              <a href={project.projectUrl} target="_blank" rel="noopener noreferrer" draggable={false}>
                <img
                  src={project.image}
                  alt={project.title}
      ...
      <div className="absolute bottom-2 flex space-x-2 z-30">
        {projects.map((_, index) => (
          <button
            key={index}
            onClick={() => setActiveIndex(index)}
            className={`h-2 rounded-full transition-all duration-300 ${index === activeIndex ? 'w-8 bg-blue-500' : 'w-2 bg-gray-500/50'
              }`}
          />
        ))}
      </div>
